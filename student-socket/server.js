...
(async () => {
  for await (let {socket} of agServer.listener('connection')) {
    ...
    // Listen for inbound messages on a socket
     (async () => {
      // Set up a loop to handle remote transmitted events.
      for await (let data of socket.receiver('excelTaskDone')) {
        agServer.exchange.transmitPublish('TaskDone', data);
      }
    })();
    ...
    (async () => {
      // Set up a loop to handle remote transmitted events.
      for await (let data of socket.receiver('excelTaskFailed')) {
        agServer.exchange.transmitPublish('TaskFailed', data);
      }
    })();
